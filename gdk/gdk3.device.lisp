...
#+liber-documentation
(setf (documentation (liber:slot-documentation "associated-device" 'device)
                     t)
...
(defun device-window-at-position (device)
 ...
 "@version{#2021-12-13}
  ...
  (with-foreign-objects ((win-x :int) (win-y :int))
    (let ((window (%device-window-at-position device win-x win-y)))
      (values window
              (cffi:mem-ref win-x :int)
              (cffi:mem-ref win-y :int)))))
...
(defun device-window-at-position-double (device)
 ...
 "@version{#2021-12-13}
  ...
  (with-foreign-objects ((win-x :double) (win-y :double))
    (let ((window (%device-window-at-position-double device win-x win-y)))
      (values window
              (cffi:mem-ref win-x :double)
              (cffi:mem-ref win-y :double)))))
...
(defcfun ("gdk_device_list_axes" device-list-axes)
    (g:list-t atom-as-string)
 #+liber-documentation
 "@version{#2021-12-13}
  @argument[device]{a @class{gdk:device} object}
