...
(test gtk-text-search-flags
  ...
  (is (eq (g:gtype "GtkTextSearchFlags")
          (g:gtype (cffi:foreign-funcall "gtk_text_search_flags_get_type" :size))))
  ;; Check names
  ...
  (is (equal '(GOBJECT:DEFINE-GFLAGS "GtkTextSearchFlags" GTK:TEXT-SEARCH-FLAGS
                       (:EXPORT T
...
(test gtk-text-iter-starts/ends-tag
  (let* ((buffer (make-instance 'gtk:text-buffer
    ...
    (is-true (gtk:text-iter-begins-tag start tag))
    (is-false (gtk:text-iter-ends-tag start tag))

    (is-false (gtk:text-iter-starts-tag end tag))
    (is-false (gtk:text-iter-begins-tag end tag))
